...
(function () {
  ...
  let chainByGame = {};
  function chainAgrees(p) { return PBBadge.chainAgrees(chainByGame, p); }
  async function loadChain(onDone) {
    chainByGame = await PBBadge.loadChain(API_BASE, SEASON);
    if (Object.keys(chainByGame).length && typeof onDone === 'function') onDone();
  }
